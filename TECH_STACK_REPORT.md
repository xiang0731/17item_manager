...
- 支持仪表盘管理：可配置过期提醒/备忘提醒显示范围（按“过期 X 天到未来 X 天”）
...
- 数据模型覆盖：
  - 用户与权限
  - 物品、分类（一级/二级）、位置（含 Emoji 图标）
  - 物品保质期字段（生产日期、保质期数值/单位）与过期日期自动推算
  - 购物清单
  - 循环提醒实例
  - 公共频道共享物品
  - 公共频道评论
...
- 网络层：`fetch` + JSON
- 物品图片大图预览支持缩放与拖拽（放大/缩小/还原 + 鼠标拖动）
- 购物清单编辑态支持状态快捷切换按钮（“转为已购买 / 转为待购买”）
- 状态持久化：`localStorage`（主题、排序、仪表盘提醒范围、视图偏好、字段显示等）
